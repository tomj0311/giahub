...
class Dalle(Toolkit):
    ...
    def create_image(self, agent: Agent, prompt: str) -> str:
        ...
        try:
            ...
            # Update the run response with the image base64 strings
            response_str = ""
            for img in response.data:
                agent.add_image(
                    Image(
                        id=str(uuid4()),
                        url="",  # Set to empty string instead of None to satisfy Pydantic string validation
                        original_prompt=prompt,
                        revised_prompt=getattr(img, "revised_prompt", None),
                        base_64_image=img.b64_json,  # Assuming ImagesResponse returns b64_json field
                    )
                )
                response_str += f"Image has been generated as base64 (length: {len(img.b64_json)})\n"
            return response_str
