...
from ai.tools.function import Function, FunctionCall, ToolCallException
...
class Model(BaseModel):
    ...
    id: Optional[str] = Field(None, alias="model")
    ...
    def _log_messages(self, messages: List[Any]) -> None:
        ...
        for m in messages:
            ...
            if hasattr(m, 'log'):
                ...
                # For dictionaries, just log the content directly
                role = m.get('role', 'unknown')
                content = m.get('content', '')
                logger.debug(f"Message ({role}): {content}")
            else:
                # Fallback for any other type
                logger.debug(f"Message (unknown format): {m}")
