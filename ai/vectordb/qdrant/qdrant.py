...
from ai.vectordb.base import VectorDb
from ai.vectordb.distance import Distance
from ai.utils.log import logger
from ai.reranker.base import Reranker
...
class Qdrant(VectorDb):
    ...
    def delete_documents_by_filename(self, filenames: List[str]) -> bool:
        """
        Delete documents from the collection by matching filenames.
        
        Args:
            filenames (List[str]): List of filenames to delete from the collection
        ...
        try:
            # Create filter to match documents by name field
            filter_conditions = []
            for filename in filenames:
                if not filename:  # Skip empty filenames
                    continue
                    
                # Match both the filename and filename without extension
                base_name = filename.split('.')[0] if '.' in filename else filename
                
                # Add conditions for exact filename match
                ...
                # Add conditions for base name match (without extension)
                if base_name != filename:
                    filter_conditions.append(
                        models.FieldCondition(
                            key="name",
                            match=models.MatchValue(value=base_name)
                        )
                    )
            ...
            # Use OR condition to match any of the filenames
            ...
            logger.debug(f"Deleted documents for filenames: {filenames}")
            return True
