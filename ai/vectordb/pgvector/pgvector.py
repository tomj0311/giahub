...
try:
    ...
    # Provide clearer guidance: the plain 'psycopg' package needs libpq on the system;
    # using the binary extra simplifies Windows installs.
    raise ImportError(
        "Missing dependencies for PgVector: install with:\n"
        "  pip install 'sqlalchemy>=2.0' 'pgvector' 'psycopg[binary]'\n"
        "If you prefer psycopg2: pip install psycopg2-binary and use URL 'postgresql+psycopg2://'.\n"
        f"Original error: {e}"
    )
...
from ai.vectordb.search import SearchType
from ai.vectordb.pgvector.index import Ivfflat, HNSW
