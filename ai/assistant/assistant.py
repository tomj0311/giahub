...
from ai.llm.references import References  # noqa: F401
from ai.memory.assistant import AssistantMemory, MemoryRetrieval, Memory  # noqa: F401
from ai.prompt.template import PromptTemplate
from ai.storage.assistant import AssistantStorage
from ai.utils.format_str import remove_indent
...
class Assistant(BaseModel):
    ...
    def update_llm(self) -> None:
        if self.llm is None:
            try:
                from ai.llm.openai import OpenAIChat
            except ModuleNotFoundError as e:
                logger.exception(e)
                logger.error(
                    "merlin uses `openai` as the default LLM. " "Please provide an `llm` or install `openai`."
                )
                exit(1)
    ...
    def _api_log_assistant_run(self):
        ...
        from ai.api_no.assistant import create_assistant_run, AssistantRunCreate
    ...
    def _api_log_assistant_event(self, event_type: str = "run", event_data: Optional[Dict[str, Any]] = None) -> None:
        ...
        from ai.api_no.assistant import create_assistant_event, AssistantEventCreate
