...
const ProjectChat = ({ user }) => {
  ...
  const startChat = async (question) => {
    try {
      ...
      // Call workflow by name "_project_answers"
      const result = await sharedApiService.makeRequest(
        '/api/workflows/by-name/_project_answers/start',
        {
          method: 'POST',
          headers: {
      ...
      if (result.success) {
        ...
        throw new Error(result.message || 'Failed to start chat workflow');
      }
    } catch (error) {
      ...
      try {
        ...
        if (result.success) {
          ...
          if (isCompleted) {
            ...
            setMessages(prev => {
              // Remove the processing message and add the response
          ...
          // Check for completed tasks (status 64) and display their results
