...
export default function EmbedderConfig({ user }) {
    ...
    const discoverComponents = async () => {
        try {
            ...
            const response = await apiCall(`/api/embedders/components?folder=ai.embeddings`, {
                headers: token ? { 'Authorization': `Bearer ${token}` } : {}
            });
            if (response.ok) {
                const data = await response.json();
                const comps = data?.components || {};
                setComponents({ functions: comps['ai.embeddings'] || [] });
            } else {
                showError('Failed to discover embedders');
            }
        } catch (error) {
            console.error('Failed to discover embedders:', error);
            showError('Failed to discover embedders');
        } finally {
    ...
    const introspectTool = async (modulePath, kind = 'tool') => {
        ...
        try {
            ...
            const response = await apiCall(`/api/embedders/introspect`, {
                method: 'POST',
                headers: {
            ...
            if (response.ok) {
                ...
                showError(`Failed to introspect embedder: ${modulePath}`);
            }
        } catch (error) {
            console.error('Failed to introspect embedder:', error);
            showError('Failed to introspect embedder');
        } finally {
    ...
    const loadCategories = async () => {
        try {
            ...
            const response = await apiCall(`/api/embedders/categories`, {
                headers: token ? { 'Authorization': `Bearer ${token}` } : {}
            });
            if (response.ok) {
                const data = await response.json();
    ...
    const loadExistingConfigs = async (page = 1, pageSize = 8) => {
        try {
            ...
            const resp = await apiCall(`/api/embedders/configs?${queryParams}`, {
                headers: {
                    ...(token ? { 'Authorization': `Bearer ${token}` } : {})
                }
            });
    ...
    useEffect(() => {
        if (!loadingDiscovery && components.functions.length === 0) {
            showWarning('No embedders discovered. Check backend logs or refresh.');
        }
    ...
    async function saveToolConfig() {
        if (!form.name || !form.tool) {
            showError('Name and embedder selection are required');
            return;
        }
        ...
        try {
            ...
            if (isEditMode && form.id) {
                resp = await apiCall(`/api/embedders/configs/${form.id}`, {
                    method: 'PUT',
                    headers: { 'Content-Type': 'application/json', ...(token ? { 'Authorization': `Bearer ${token}` } : {}) },
                    body: JSON.stringify(configToSave)
                });
            } else {
                resp = await apiCall(`/api/embedders/configs`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json', ...(token ? { 'Authorization': `Bearer ${token}` } : {}) },
                    body: JSON.stringify(configToSave)
                });
            ...
            showSuccess(`Embedder configuration "${form.name}" ${action} successfully`);
            setSaveState({ loading: false });
            loadExistingConfigs();
            loadCategories();
    ...
    async function deleteToolConfig(id, configName) {
        ...
        const confirmed = await showDeleteConfirmation({
            ...
            itemType: 'embedder configuration',
        });
        ...
        try {
            ...
            const resp = await apiCall(`/api/embedders/configs/${id}`, {
                method: 'DELETE',
                headers: { ...(token ? { 'Authorization': `Bearer ${token}` } : {}) }
            });
            const data = await resp.json().catch(() => ({}));
            ...
            showSuccess('Embedder configuration deleted');
    ...
    return (
        <Box>
            ...
            <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3 }}>
                <Box>
                    ...
                    <Typography variant="body1" color="text.secondary">
                        Discover embedders from ai.embeddings and configure their parameters.
                    </Typography>
                </Box>
                ...
                <CardContent>
                    <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
                        <Typography variant="h6">
                            Embedder Configs ({pagination.total} total, showing {existingConfigs.length} on page {pagination.page + 1})
                        </Typography>
                    </Box>
            ...
            <Dialog open={dialogOpen} onClose={() => setDialogOpen(false)} maxWidth="md" fullWidth>
                <DialogTitle>{isEditMode ? 'Edit Embedder Configuration' : 'Create Embedder Configuration'}</DialogTitle>
                    ...
                    <Stack spacing={1}>
                        ...
                        <Box sx={{ display: 'flex', gap: 1, alignItems: 'center' }}>
                            <Autocomplete
                                ...
                                loadingText="Loading embedders…"
                                onChange={(_, v) => {
                                    setForm(f => ({ ...f, tool: v || '', tool_params: {} }));
                                    ensureIntrospection(v, 'tool');
                                }}
                                renderInput={(params) => <TextField {...params} label="Select Embedder" />}
                            />
                        ...
                        {toolIntro && (
                            <Box>
                                <Typography variant="subtitle2" sx={{ mb: 1 }}>
                                    Embedder Parameters ({toolIntro.class_name})
                                </Typography>
